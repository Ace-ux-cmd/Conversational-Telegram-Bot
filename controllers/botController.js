...
async function processUserRequest(bot, currentUser) {
    ...
    // Get AI content
    const aiResponse = await getAIResponse(currentUser);

    // Send response to user
    if (aiResponse) {
        bot.sendMessage(currentUser.userId, aiResponse, { 
            reply_to_message_id: currentUser.msgId 
        });
    } else {
        bot.sendMessage(currentUser.userId, "Currently busy rn, TTYL", { 
            reply_to_message_id: currentUser.msgId 
        });
    }
}
